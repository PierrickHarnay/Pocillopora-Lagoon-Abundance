...
#Class 2 >1 - 4cm in diameter = >0.7854- 2.2567 cm2
#Class 3 >4 - 6cm in diameter = >2.2567 - 2.7639 cm2
#Class 4 >6 - 10cm in diameter = >2.7639- 3.5682 cm2
...
#Class 2 0 - 1cm in diameter = >0.7854- 2.2567 cm2
c2A <- pi*(c2/2)^2

#Class 3 0 - 1cm in diameter = >2.2567 - 2.7639 cm2
c3A <- pi*(c3/2)^2

#Class 4 0 - 1cm in diameter = >2.7639- 3.5682 cm2
c4A <- pi*(c4/2)^2

```

relative abundance by area cm2 (script took on Relative abundace Lifestage)
...
#Load Data and QC
```{r}
data_size_cm2 <- read.csv("Data/POC_RA_Size.csv")
...
Rel.Abund$Site <- factor(Rel.Abund$Site, levels=c("S1", "S2", "S3", "S4", "S5", "S11", "S15"))
Rel.Abund$Site
...
ggsave("Output/S1_RA.jpg", S1.plot, height=4, width=5)
ggsave("Output/S2_RA.jpg", S2.plot, height=4, width=5)
