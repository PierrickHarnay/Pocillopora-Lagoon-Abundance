...
```{r}
#I want to visualize the PCA in 4 differents way, meaning one PCA per size class
#first we need to create 4 differents datafile where the size as been subset 
#creation of the dataset 

Class_0_2_cm <- subset(relative_abundance.LS, LifeStage == "0-2cm")
Class_2_5_cm <- subset(relative_abundance.LS, LifeStage == ">2cm-5cm")
Class_5_14_cm <- subset(relative_abundance.LS, LifeStage == ">5cm-14cm")
Class_14_cm <- subset(relative_abundance.LS, LifeStage == ">14cm")

#create site x species matrix 
mat_0_2_cm <- with(Class_0_2_cm, tapply(Relative_Abundance, list(Site, Species), sum, na.rm=TRUE))

#removing the NA in the data set
mat_0_2_cm[is.na(mat_0_2_cm)] <- 0

#doing tranformation with Hellinger because we have a relative abundance 
mat_0_2_cm.hel <- decostand(mat_0_2_cm, method="hellinger")

#PCA
pca1_0_2_cm <- rda(mat_0_2_cm.hel)

#ploting PCA 
plot(pca1_0_2_cm, display="sites", type="n")  # empty plot
points(pca1_0_2_cm, display="sites", pch=21, bg="lightblue", cex=1.2)
text(pca1_0_2_cm, display="species", col="red", cex=0.8)

#test with class_5_14_cm
mat_5_14_cm <- with(Class_5_14_cm, tapply(Relative_Abundance, list(Site, Species), sum, na.rm=TRUE))

#removing the NA in the data set
mat_5_14_cm[is.na(mat_5_14_cm)] <- 0

#doing tranformation with Hellinger because we have a relative abundance 
mat_5_14_cm.hel <- decostand(mat_5_14_cm, method="hellinger")

#PCA
pca1_5_14_cm <- rda(mat_5_14_cm.hel)

#ploting PCA 
plot(pca1_5_14_cm, display="sites", type="n")  # empty plot
points(pca1_5_14_cm, display="sites", pch=21, bg="lightblue", cex=1.2)
text(pca1_5_14_cm, display="species", col="red", cex=0.8)
```
